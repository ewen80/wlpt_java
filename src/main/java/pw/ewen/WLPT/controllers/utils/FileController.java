...
public class FileController {
     ...
     * 上传文件按照年月分文件夹存储
     * @param file 文件流
     * @apiNote  {name: "原始文件名", path: "含路径的文件名", status: "result", date: "上传时间"}, result: "done"完成 "error"错误
     */
    @PostMapping(value = "/upload", produces = "application/json")
    public UploadReturn upload(@RequestParam("file") MultipartFile file)  {
        UploadReturn uploadReturn = new UploadReturn();
        try{
            uploadReturn.name = file.getOriginalFilename();
            uploadReturn.path = fileService.upload(file);
